...
const night = new Keyboard('div');
const toggleCircle = new Keyboard('div');
const keyboardWrap = new Keyboard('div');
const text = new Keyboard('textarea');
const changeColor = new Keyboard('div');
const colors = new Keyboard('div');
const colorsInput = new Keyboard('input');
const keyboardLights = new Keyboard('div');
const help = new Keyboard('div');
...
const helpInner = document.querySelector('.help');
helpInner.innerHTML = `
  <h1 class="help__text">
...
function innerText() {
  ...
  keysData.forEach((item, i) => {
    ...
    } else if (JSON.parse(localStorage.getItem('MyObj')) && JSON.parse(localStorage.getItem('MyObj')).lang === 'ru') {
      block.textContent = keys[i].ru;
    } else if (JSON.parse(localStorage.getItem('MyObj')) && JSON.parse(localStorage.getItem('MyObj')).lang === 'eng') {
      block.textContent = keys[i].eng;
    } else {
      block.textContent = keys[i].eng;
    }
...
function capsLockOn() {
  keysData.forEach((item) => {
    const key = item;
    if (!obj.CapsLock) {
      if (!notAddToTextArea.includes(key.textContent)) {
        key.textContent = key.textContent.toUpperCase();
      }
    } else if (!notAddToTextArea.includes(key.textContent)) {
